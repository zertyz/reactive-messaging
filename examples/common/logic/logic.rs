...
impl Umpire {
    ...
    pub fn new(config: MatchConfig, server_player: Players) -> Self {
                ...
                GameStates::WaitingForService { attempt } => {
                    self.consider_service_soft_fault(action, attempt)
                        .map(|soft_fault| {
                            ...
                            PingPongEvent::SoftFault(soft_fault)
                        })
                        .unwrap_or_else(|| {
                            self.consider_hard_fault(action)
                                .map(|hard_fault| self.compute_lost_score(hard_fault))
                                .unwrap_or_else(|| {
                                    self.state = GameStates::Rally;
                                    self.progress_on_the_rally(TurnFlipEvents::SuccessfulService(action))
                                })
                        })
                },
                GameStates::Rally => {
                    self.consider_hard_fault(action)
                        .map(|hard_fault| self.compute_lost_score(hard_fault))
                        .unwrap_or_else(|| self.progress_on_the_rally(TurnFlipEvents::SuccessfulRebate(action)))
                }
    ...
    fn progress_on_the_rally(&mut self, turn_flip_event: TurnFlipEvents) -> PingPongEvent {
        self.next_turn_player();
        PingPongEvent::TurnFlip(turn_flip_event)
    }
    ...
    fn compute_lost_score(&mut self, hard_fault: FaultEvents) -> PingPongEvent {
...
mod tests {
    ...
    fn happy_path_game() {
        ...
        assert_eq!(umpire.process_turn(Players::OneSelf, PlayerAction { lucky_number: config.net_touch_probability }),
                   PingPongEvent::Score { point_winning_player: Players::Opponent, opponent_fault: FaultEvents::NetBlock },
        ...
        assert_eq!(umpire.process_turn(Players::Opponent, PlayerAction { lucky_number: config.ball_out_probability }),
                   PingPongEvent::Score { point_winning_player: Players::OneSelf, opponent_fault: FaultEvents::BallOut },
...
//println!("\n");
        let score = loop {
            ...
            let action = PlayerAction { lucky_number: pick_a_lucky_number() };
            let umpire_1_event = player_1_umpire.process_turn(turn_player, action.clone());
            let umpire_2_event = player_2_umpire.process_turn(next_turn_player(turn_player), action.clone());
...
//println!("Game Manager 2 state: {:?}", player_2_umpire.state);
    ...
    fn assert_game_events(umpire_1_event: PingPongEvent, umpire_2_event: PingPongEvent) {
        match umpire_1_event {
            PingPongEvent::Score { point_winning_player: point_winning_player_1, opponent_fault: opponent_fault_1 } => {
                if let PingPongEvent::Score { point_winning_player: point_winning_player_2, opponent_fault: opponent_fault_2 } = umpire_2_event {
    ...
    /// Creates a random player action
    fn pick_a_lucky_number() -> f32 {
        rand::thread_rng().gen()
    }
