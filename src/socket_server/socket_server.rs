//! Provides [new_socket_server!()] macro for instantiating servers, which may then be started with [spawn_unresponsive_processor!()] or [spawn_responsive_processor!()],
//! with each taking a different kind of reactive processor logic -- that will receive and return `Stream`s.\
//! Both reactive processing logic kinds will take in a `Stream` as parameter and should return another `Stream` as output:
...
/// Instantiates & allocates resources for a [GenericSocketServer], ready to be later started by [spawn_unresponsive_processor!()] or [spawn_responsive_processor!()].\
/// Params:
...
#[macro_export]
macro_rules! spawn_unresponsive_processor {
    ($socket_server:                expr,
...
pub use spawn_unresponsive_processor;


/// See [GenericSocketServer::spawn_responsive_processor()]
#[macro_export]
macro_rules! spawn_responsive_processor {
    ($socket_server:                expr,
     $connection_events_handler_fn: expr,
     $dialog_processor_builder_fn:  expr) => {{
        use crate::socket_server::SocketServer;
...
pub use spawn_responsive_processor;
...
///                         This [Uni] will execute the given server logic for each incoming message (see how it is used in [new_socket_server!()]);
...
GenericSocketServer<CONFIG, RemoteMessages, LocalMessages, ProcessorUniType, SenderChannel> {
    ...
    ///                                        LocalMessages: [your type for messages produced by the SERVER],
    ///                                        SenderChannel: FullDuplexUniChannel<ItemType=LocalMessages, DerivedItemType=LocalMessages> + Send + Sync>
    ...
    pub fn shutdown(mut self, timeout_ms: u32) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        match self.server_shutdown_signaler.take() {
            Some(server_sender) => {
                warn!("SocketServer: Shutdown asked & initiated for server @ {}:{} -- timeout: {timeout_ms}ms", self.interface_ip, self.port);
...
mod tests {
    ...
    use crate::{new_responsive_socket_client, ron_deserializer, ron_serializer};
    ...
    async fn doc_usage() -> Result<(), Box<dyn std::error::Error + Sync + Send>> {
        ...
        let mut server = new_socket_server!(
            ...
            DummyClientAndServerMessages);
        spawn_unresponsive_processor!(server,
    ...
    async fn shutdown_process() {
        ...
        let _client = new_responsive_socket_client!(
            ConstConfig::default(),
            "127.0.0.1",
            PORT,
            DummyClientAndServerMessages,
