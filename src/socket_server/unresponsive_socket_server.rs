...
use crate::socket_server::socket_server_types::{SocketServerController, UnresponsiveSocketServerAssociatedTypes};
...
/// to be sent to the clients. To send messages, call to [Peer::send()] must be done.\
...
impl<const CONFIG:   usize,
     ...
     ProcessorUniType:  GenericUni<ItemType=RemoteMessages>                                         + Send + Sync + 'static,
     SenderChannelType: FullDuplexUniChannel<ItemType=LocalMessages, DerivedItemType=LocalMessages> + Sync + Send + 'static>
UnresponsiveSocketServer<CONFIG, RemoteMessages, LocalMessages, ProcessorUniType, SenderChannelType> {
...
UnresponsiveSocketServer<CONFIG, RemoteMessages, LocalMessages, ProcessorUniType, SenderChannelType> {
    ...
    async fn spawn_unresponsive_processor<OutputStreamItemsType:                                                                                                                                                                                                                Send + Sync + Debug + 'static,
                                         ...
                                         -> Result<Box<dyn SocketServerController>, Box<dyn Error + Sync + Send + 'static>> {
